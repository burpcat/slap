...
| `python slap.py dashboard` | Starts the localhost dashboard at `http://127.0.0.1:5050` — today/week stats, engagement metrics, replies needing triage (tag as real/OOO/not-interested — OOO and not-interested both trigger a real, account-wide GMass unsubscribe, not just a label), bounces & blocks (with the real reason text), pipeline, recent run history, and a link to the all-campaigns **Reach-outs** page (`/reachouts`, filterable — also supports triage tagging and bounce-address correction directly from each row, not just read-only). |
...
| `python slap.py bounced` | Fixes bounced/blocked recipients: paste a corrected email address for each and it resends the full sequence to it as a brand-new recipient, recovering the original subject/body/attachment automatically — no re-pasting the drop. Same action is also on each bounced row in the Reach-outs page. See [`USAGE.md`](USAGE.md) for the full flow. |

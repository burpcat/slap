...
own `cadence` length (`slap.config._validate_stage_files`), so the cadence
must be chosen (step 2) before the follow-up bodies can be authored (step 5)
...
from slap.prompts import ask, ask_cadence, ask_yn, placeholder_pdf, read_paste
...
def _ask_persona(global_config, *, read_line=input) -> str:
    ...
    display.plain("\nAvailable personas (content/register — cadence is chosen next, per campaign):")
    for n in names:
        display.plain(f"  {n} (suggested cadence: {global_config.personas[n]})")
    while True:
...
def _ask_resumes(global_config, *, read_line=input) -> list:
    """Which config.yaml send_tags this static campaign offers under `resumes:`
    (first = default). Résumés now live centrally in docs_dir, referenced by tag —
    the wizard picks from the already-defined tags rather than copying a PDF into
    the campaign folder."""
    tags = sorted(global_config.send_tags)
    if not tags:
        raise OnboardError(
            "This is a static (non-LaTeX) campaign, but config.yaml defines no `send_tags:` to attach. "
            "Put a résumé in your docs_dir and add a send_tags entry pointing at it, then rerun."
        )
    display.plain(f"\nAvailable résumé tags (config.yaml send_tags): {tags}")
    while True:
        raw = ask("Which résumé tag(s) should this campaign offer (comma-separated, first = default)",
                  default=tags[0], read_line=read_line).strip()
        chosen = [t.strip() for t in raw.split(",") if t.strip()]
        if not chosen:
            display.warn("  Pick at least one tag.")
            continue
        unknown = [t for t in chosen if t not in global_config.send_tags]
        if unknown:
            display.warn(f"  Unknown tag(s) {unknown} — choose from {tags}.")
            continue
        return chosen


def _ask_latex_and_attachment(global_config, *, read_line=input) -> dict:
    """Returns {"latex_enabled", "attachment_name", "resumes"} — `resumes` is []
    for a latex-enabled campaign (the résumé is pasted fresh per recipient at
    send time) and a non-empty list of config.yaml send_tags for a static one."""
    ...
    if latex_enabled:
        return {"latex_enabled": True, "attachment_name": attachment_name, "resumes": []}
    resumes = _ask_resumes(global_config, read_line=read_line)
    return {"latex_enabled": False, "attachment_name": attachment_name, "resumes": resumes}


# --- step 8: write + validate --------------------------------------------------

def _write_campaign(name: str, *, persona: str, cadence: list, raw_initial_text: str, stage_bodies_raw: list,
                     fields: dict, latex_enabled: bool, attachment_name: str, resumes: list) -> Path:
    dest = CAMPAIGNS_DIR / name
    ...
    # No per-campaign résumé.pdf — static campaigns reference config.yaml
    # send_tags (docs_dir), so there is nothing to write into the folder here.
    return dest
...
def run_onboard_campaign(*, read_line=input) -> None:
    ...
    # Cadence is now a per-campaign choice (no longer derived from the persona map);
    # the chosen persona's stages are offered only as a suggested starting point.
    cadence = ask_cadence("Follow-up cadence", default=global_config.personas[persona], read_line=read_line)
    ...
    display.plain(
        f"\nCadence {cadence} → {len(cadence)} follow-up stage(s); paste one body per stage."
    )
    ...
    rest = _ask_latex_and_attachment(global_config, read_line=read_line)

    dest = _write_campaign(
        name, persona=persona, cadence=cadence, raw_initial_text=raw_initial_text,
        stage_bodies_raw=stage_bodies_raw, fields=fields, **rest,
    )

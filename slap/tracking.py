...
CREATE TABLE IF NOT EXISTS events (
    ...
    gmass_draft_id TEXT,
    -- The RFC822 Message-ID of an SMTP send (slap.smtp), replacing the GMass
    -- transport's gmass_campaign_id/gmass_draft_id. It is BOTH the threading
    -- key (a follow-up/OOO resend sets In-Reply-To to the initial send's
    -- message_id) AND the reply-match key (slap.imap matches an inbound
    -- reply's In-Reply-To/References against the set of sent message_ids).
    -- Added additively (expand-migrate-contract): the gmass_* columns above
    -- stay for historical rows during the GMass->SMTP migration and are
    -- dropped once no writer references them.
    message_id TEXT,
    meta TEXT
);
"""
...
CREATE TABLE IF NOT EXISTS recipients (
    ...
    -- The initial send's Message-ID for this recipient — the parent a
    -- follow-up/OOO resend threads into (In-Reply-To). Under GMass this role
    -- was played by last_gmass_campaign_id (campaignIdToReplyTo); message_id
    -- supersedes it for SMTP sends. Populated by the `sent`/`requeued`
    -- handlers below; added additively alongside the gmass_* column.
    message_id TEXT,
    first_sent_at TEXT,
    last_event_at TEXT,
    replied_at TEXT,
...
def _migrate_add_message_id_columns(conn: sqlite3.Connection) -> None:
    """One-time additive migration for the `message_id` columns on both
    `events` and `recipients` (the SMTP threading + reply-match key that
    supersedes the GMass transport's gmass_* columns — see the CREATE TABLE
    comments and this module's docstring). Plain `ALTER TABLE ... ADD COLUMN`,
    exactly like `_migrate_recipients_cadence_column` — no CHECK constraint is
    involved, so none of `_migrate_events_check_constraint`'s rename/recreate
    dance is needed.

    Idempotent via `PRAGMA table_info`, mirroring the cadence migration. A
    fresh db has neither table yet at this point (this runs before `_SCHEMA`'s
    own `CREATE TABLE IF NOT EXISTS`), so both blocks below no-op and `_SCHEMA`
    creates the tables moments later already including `message_id`. An
    already-existing db missing the column on either table gets it added here.

    Note: `_migrate_events_check_constraint`'s INSERT...SELECT deliberately does
    NOT copy `message_id` — it only ever runs on dbs predating `linkedin_gate`,
    which necessarily also predate `message_id`, so there is nothing to copy.
    When a FUTURE event-type literal bumps that migration's guard, its
    INSERT...SELECT must be extended to carry `message_id` across."""
    for table in ("events", "recipients"):
        cols = {row[1] for row in conn.execute(f"PRAGMA table_info({table})")}
        if not cols or "message_id" in cols:
            continue  # no such table yet (fresh db), or already migrated
        conn.execute(f"ALTER TABLE {table} ADD COLUMN message_id TEXT")
    conn.commit()
...
def connect(path: Path = DB_PATH) -> sqlite3.Connection:
    ...
    _migrate_add_message_id_columns(conn)
...
def append_event(conn: sqlite3.Connection, *, type: str, recipient: str = None,
                  ...
                  gmass_draft_id: str = None, message_id: str = None, meta: dict = None,
                  timestamp=None) -> int:
    ...
    `message_id` is the SMTP send's RFC822 Message-ID (see the events-table
    comment) — recorded on `sent`/`requeued` events by slap.runner so later
    follow-ups can thread into it and slap.imap can match inbound replies. The
    gmass_* params remain during the GMass->SMTP migration for historical
    callers; new SMTP sends populate message_id instead."""
    ...
    cur = conn.execute(
        ...
        "gmass_campaign_id, gmass_draft_id, message_id, meta) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (ts, recipient, campaign, type, stage, gmass_campaign_id, gmass_draft_id,
    ...
    _apply_event_to_cache(conn, {
        ...
        "gmass_draft_id": gmass_draft_id, "message_id": message_id, "meta": meta,
    })
    conn.commit()
    return event_id
...
def _upsert_recipient(conn, recipient, *, campaign=None, persona=None, status=None,
                       current_stage=None, last_gmass_campaign_id=None, message_id=None,
    ...
    conn.execute(
        ...
        INSERT INTO recipients
            (recipient, campaign, persona, status, current_stage,
             last_gmass_campaign_id, message_id, first_sent_at, last_event_at, replied_at, cadence)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(recipient) DO UPDATE SET
            ...
            message_id = COALESCE(excluded.message_id, recipients.message_id),
        ...
        (recipient, campaign, persona, status, current_stage, last_gmass_campaign_id,
         message_id, first_sent_at, last_event_at, replied_at, cadence),
    )
...
def _apply_event_to_cache(conn: sqlite3.Connection, event: dict) -> None:
    ...
    if etype == "queued":
        ...
        _upsert_recipient(conn, recipient, campaign=campaign, status="active",
                           current_stage=event["stage"],
                           last_gmass_campaign_id=event["gmass_campaign_id"],
                           message_id=event.get("message_id"), last_event_at=ts)

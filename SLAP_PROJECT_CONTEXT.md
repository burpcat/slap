...
```
slap.py                     # entry point — real CLI surface (10 commands, not the brief's original 6):
                             #   list | send <campaign> [--now] | dashboard | doctor | init |
                             #   domains | rebuild | runner | plist | cleanup [--confirm] [--min-days-idle N]
slap/                       # package:
    ...
    dashboard.py + dashboard_templates/dashboard.html + reachouts.html   # Flask app, both pages
...
conftest.py, tests/         # pytest suite (462 tests passing as of this snapshot; `pytest -m slow` for real-xelatex/real-socket tests)
```
...
contacted, Pipeline, Today's runs — plus a link to `/reachouts`.
